...
// Mocks the barrel, not the module: piDatasources reaches core through
// `@siesta/core`, so that is the specifier Vitest has to intercept. The factory
// only needs `accountsService` — the one symbol the module under test imports.
vi.mock("@siesta/core", () => ({
  accountsService: {
