...
// The manifest is this workspace's; the publish metadata (repository, homepage,
// bugs — where the <TODO_*> placeholders live) is the monorepo root's. Reading
// the workspace package.json for placeholders would pass vacuously.
const appRoot = dirname(dirname(fileURLToPath(import.meta.url)));
const repoRoot = dirname(dirname(appRoot));
const pkg = readFileSync(join(repoRoot, "package.json"), "utf8");
const manifest = readFileSync(
  join(appRoot, "io.github.aguilarguisado.siestadeck.sdPlugin", "manifest.json"),
  "utf8",
);
